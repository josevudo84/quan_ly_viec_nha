...
function openModal(type, item = null) {
  ...
  if (type === 'users') {
    ...
    if (currentUser.role === 'Admin') {
      roleSelect = `
        <label class="block text-xs font-bold text-muted mb-2 uppercase tracking-wider">Vai trò</label>
    ...
    body.innerHTML = `
      <label class="block text-xs font-bold text-muted mb-2 uppercase tracking-wider">Tên đăng nhập</label>
      <input id="inp-username" type="text" value="${item ? item.username : ''}" placeholder="username" class="w-full bg-[#16181D] border border-borderline rounded-xl px-4 py-3 text-white text-sm outline-none mb-3" ${item ? 'disabled' : ''}>
      <label class="block text-xs font-bold text-muted mb-2 uppercase tracking-wider">Tên hiển thị</label>
      <input id="inp-name" type="text" value="${item ? item.name : ''}" placeholder="Tên" class="w-full bg-[#16181D] border border-borderline rounded-xl px-4 py-3 text-white text-sm outline-none mb-3">
      <label class="block text-xs font-bold text-muted mb-2 uppercase tracking-wider">Mật khẩu</label>
      <input id="inp-password" type="text" value="${item ? item.password : ''}" placeholder="Mật khẩu" class="w-full bg-[#16181D] border border-borderline rounded-xl px-4 py-3 text-white text-sm outline-none mb-3">
      ${roleSelect}
      <label class="block text-xs font-bold text-muted mb-2 uppercase tracking-wider">Điểm hiện tại</label>
      <input id="inp-points" type="number" value="${item ? item.points : 0}" class="w-full bg-[#16181D] border border-borderline rounded-xl px-4 py-3 text-white text-sm outline-none mb-3">
    `;
    saveBtn.onclick = () => saveData(type, item);

  } else if (type === 'tasks') {
    body.innerHTML = `
      <label class="block text-xs font-bold text-muted mb-2 uppercase tracking-wider">Tên công việc</label>
      <input id="inp-task-name" type="text" value="${item ? item.task_name : ''}" placeholder="VD: Rửa chén" class="w-full bg-[#16181D] border border-borderline rounded-xl px-4 py-3 text-white text-sm outline-none mb-3">
      <label class="block text-xs font-bold text-muted mb-2 uppercase tracking-wider">Tần suất</label>
      <select id="inp-frequency" class="w-full bg-[#16181D] border border-borderline rounded-xl px-4 py-3 text-white text-sm outline-none mb-3">
      ...
      <label class="block text-xs font-bold text-muted mb-2 uppercase tracking-wider">Lịch (0=CN, 1-6=T2-T7 | Tuần 1-4 cho Monthly)</label>
      <input id="inp-schedule" type="number" value="${item ? item.schedule : 0}" class="w-full bg-[#16181D] border border-borderline rounded-xl px-4 py-3 text-white text-sm outline-none mb-3">
      <label class="block text-xs font-bold text-muted mb-2 uppercase tracking-wider">Điểm thưởng</label>
      <input id="inp-points" type="number" value="${item ? item.points : 10}" class="w-full bg-[#16181D] border border-borderline rounded-xl px-4 py-3 text-white text-sm outline-none mb-3">
      <label class="block text-xs font-bold text-muted mb-2 uppercase tracking-wider">Điểm phạt</label>
      <input id="inp-penalty" type="number" value="${item ? item.penalty : 0}" class="w-full bg-[#16181D] border border-borderline rounded-xl px-4 py-3 text-white text-sm outline-none mb-3">
      <label class="block text-xs font-bold text-muted mb-2 uppercase tracking-wider">Icon (FontAwesome class)</label>
      <input id="inp-icon" type="text" value="${item ? (item.icon || '') : 'fa-solid fa-clipboard-list'}" placeholder="fa-solid fa-broom" class="w-full bg-[#16181D] border border-borderline rounded-xl px-4 py-3 text-white text-sm outline-none mb-3">
    `;
    saveBtn.onclick = () => saveData(type, item);

  } else if (type === 'rewards') {
    body.innerHTML = `
      <label class="block text-xs font-bold text-muted mb-2 uppercase tracking-wider">Tên phần thưởng</label>
      <input id="inp-reward-name" type="text" value="${item ? item.reward_name : ''}" placeholder="VD: Ăn kem" class="w-full bg-[#16181D] border border-borderline rounded-xl px-4 py-3 text-white text-sm outline-none mb-3">
      <label class="block text-xs font-bold text-muted mb-2 uppercase tracking-wider">Chi phí (điểm)</label>
      <input id="inp-cost" type="number" value="${item ? item.cost : 50}" class="w-full bg-[#16181D] border border-borderline rounded-xl px-4 py-3 text-white text-sm outline-none mb-3">
      <label class="block text-xs font-bold text-muted mb-2 uppercase tracking-wider">Icon (FontAwesome class)</label>
      <input id="inp-icon" type="text" value="${item ? (item.icon || '') : 'fa-solid fa-gift'}" placeholder="fa-solid fa-ice-cream" class="w-full bg-[#16181D] border border-borderline rounded-xl px-4 py-3 text-white text-sm outline-none mb-3">
    `;
    saveBtn.onclick = () => saveData(type, item);
  }
...
async function deleteData(type, id) {
  ...
  let error = null;
  if (type === 'users') {
    const res = await supabaseClient.from('users').delete().eq('username', id);
    error = res.error;
  } else if (type === 'tasks') {
    const res = await supabaseClient.from('tasks').delete().eq('id', id);
    error = res.error;
  } else if (type === 'rewards') {
    const res = await supabaseClient.from('rewards').delete().eq('id', id);
    error = res.error;
  }
...
async function saveData(type, existingItem) {
  showLoading(true);
  let error = null;

  if (type === 'users') {
    const payload = {
      username: document.getElementById('inp-username').value.trim().toLowerCase(),
      name: document.getElementById('inp-name').value.trim(),
      password: document.getElementById('inp-password').value.trim(),
      points: parseInt(document.getElementById('inp-points').value) || 0,
    };
    ...
    if (existingItem) {
      const res = await supabaseClient.from('users').update(payload).eq('username', existingItem.username);
      error = res.error;
    } else {
      if (!payload.role) payload.role = 'User';
      const res = await supabaseClient.from('users').insert([payload]);
      error = res.error;
    }

  } else if (type === 'tasks') {
    const payload = {
      task_name: document.getElementById('inp-task-name').value.trim(),
      frequency: document.getElementById('inp-frequency').value,
      schedule: parseInt(document.getElementById('inp-schedule').value) || 0,
      points: parseInt(document.getElementById('inp-points').value) || 10,
      penalty: parseInt(document.getElementById('inp-penalty').value) || 0,
      icon: document.getElementById('inp-icon').value.trim() || 'fa-solid fa-clipboard-list',
    };

    if (!payload.task_name) {
      showLoading(false);
      return showToast('Vui lòng nhập tên công việc!', 'error');
    }

    if (existingItem) {
      const res = await supabaseClient.from('tasks').update(payload).eq('id', existingItem.id);
      error = res.error;
    } else {
      const res = await supabaseClient.from('tasks').insert([payload]);
      error = res.error;
    }

  } else if (type === 'rewards') {
    const payload = {
      reward_name: document.getElementById('inp-reward-name').value.trim(),
      cost: parseInt(document.getElementById('inp-cost').value) || 50,
      icon: document.getElementById('inp-icon').value.trim() || 'fa-solid fa-gift',
    };

    if (!payload.reward_name) {
      showLoading(false);
      return showToast('Vui lòng nhập tên phần thưởng!', 'error');
    }

    if (existingItem) {
      const res = await supabaseClient.from('rewards').update(payload).eq('id', existingItem.id);
      error = res.error;
    } else {
      const res = await supabaseClient.from('rewards').insert([payload]);
      error = res.error;
    }
  }
